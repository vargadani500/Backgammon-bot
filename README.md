...
The bot `ai_bot` and its 2-ply version `hard_ai_bot` are using neural networks provided by PyTorch to predict the value of the state of the board. For the training we used reinforcement learning with the method of TD-learning. The training datasets were generated by playing matches against `greedy_bot` and via self-playing. For constructing the neural network's structure we followed Gemini's advice.
...
<p align="center">
  ...
  <img src="GIFs/hard_bot%20vs%20random_bot.gif" alt="Hard Bot vs Random Bot" width="48%">
  <br>
  <em>Here are the two best bots against each other, than against random moves</em>
</p>
